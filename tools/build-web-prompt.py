...
comments you need, and never guess at Basecamp content you could not read.
**Ask which project the work concerns** before drafting — there is no project
registry here to resolve it from. The readiness gate and the locked 11-field
format apply exactly as written — Paste Mode changes how you get context, never
the standard.
...
BUNDLES = {
    "basecamp-todo-coach": {
        ...
        "always_replace": {
            ...
            # Browser chat has no filesystem, so the registry and the local
            # folder cannot exist. Left as file paths, these would send the
            # agent looking for files it can never have.
            "`references/project-registry.md`": "the project registry (unavailable in this mode — ask the user which project)",
            "`references/local-context.md`": "the project's local folder (unavailable in this mode)",
        },
        ...
        "targets": {
            "gem": {
                ...
                "hard_limit": False,
                "instructions": [
                    ("SKILL.md", ALL_SECTIONS, ["Operating Modes — Preflight", "Which project? Resolve it first"]),
