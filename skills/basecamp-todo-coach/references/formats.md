...
## 5. Backlog Health Summary

Added 2026-09-17. The output of a **bulk** readiness review — every todo in a
project rather than one.

The readiness gate runs underneath exactly as it does for a single todo. What
changes is the framing: the reader is a stakeholder asking *"where does this
initiative stand?"*, not a PIC asking *"is this todo well-formed?"*. So the
summary reports the aggregate and what to do about it, not a verdict per item.

```text
[Backlog Health - Module / Area - DD Month YYYY]

Ready to work on: [n of total]
- [Todo title] — [PIC]

Not ready: [n]
- [Todo title] — [the single missing thing, not a full review]

Blocked or waiting: [n]
- [Todo title] — waiting on [Named person] since [DD Month YYYY]

Needs a decision:
- [Question, and who must answer it]

Epic-level gaps:
- [Missing Epic Brief, no Validation PIC named, no Done Criteria, …]

Suggested focus:
- [The one or two things that would unblock the most work]
```

Rules:

- **Counts first.** "9 of 17 ready" is the sentence a stakeholder actually
  wants; the lists are the supporting detail.
- **One line per todo, naming the single biggest gap.** Not a four-part review —
  that is what the per-todo workflow is for. Offer to open any of them properly.
- **Name people and dates for anything blocked.** "Waiting on Finance" tells a
  stakeholder nothing they can act on.
- **Epic-level gaps are worth more than todo-level ones.** A missing Epic Brief
  or an unnamed Validation PIC affects every todo under it; say so before
  listing individual items.
- **Suggested focus is at most two items.** The point of a health summary is to
  make the next move obvious, not to enumerate everything wrong.
- A todo that predates a field (`Tip` from 2026-07-28, `Dependencies` from
  2026-07-29) is **not** counted as not-ready for lacking it. See Review Mode.

### Do not turn this into a work plan

The summary reports state. It does not create todos to fix what it finds, and it
does not rewrite the todos it reviewed. If the user wants a gap turned into
work, run the individual todo workflow so the gate applies properly.
