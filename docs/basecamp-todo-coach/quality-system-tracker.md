...
| 4 | Create Gemini Gem instruction | In Progress | v4.11 — `Dependencies` field added (2026-07-29); reopened for testing |
...
| 6 | Create How-To guide for mentees | Not started | Depends on #4 and #5 |
| 7 | Create presentation slides | Optional / Not started | Can be created after guidance is stable |
| 8 | Create universal agent skill (Agent/CLI + Paste mode) | In Progress | v2.8 built under `skills/basecamp-todo-coach/` in the `buspro-skills` repo, tracking instruction v4.12; own repo, installed via `npx skills`; browser-chat bundles generated from the same source; re-sync when #4 locks |
...
Status: **In Progress** — design record `docs/basecamp-todo-coach/design-record.md` (v2.7; filename now stable, version lives in the header); built artifact `skills/basecamp-todo-coach/` (v2.7)
