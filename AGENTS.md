...
## Text two skills both need

`npx skills` installs one skill directory and nothing else, so a skill cannot
reference another skill's files. Anything both need must exist in both.

**Do not copy it by hand.** Put the canonical text in `shared/`, list the skills
that need it in `tools/sync-shared.py`, and run the script. The copies carry a
`GENERATED` header; edit the canonical file and re-run, never the copies.

```bash
python3 tools/sync-shared.py           # write the copies
python3 tools/sync-shared.py --check   # verify they are current
```

`publish-public.sh` refuses to publish stale copies. This exists because
hand-copying has already produced three drifted duplicates in this repo.
